...
class DataStore: ObservableObject {
    ...
    // MARK: - SeatingPosition Operations
        ...
        // Batch addition of seating positions
        func addSeatingPositions(_ positions: [SeatingPosition]) {
            guard !positions.isEmpty else { return }
            
            do {
                // Save each position and collect the saved results
                var savedPositions: [SeatingPosition] = []
                
                // Use GRDB transaction for better performance
                try AppDatabase.shared.write { db in
                    for position in positions {
                        let positionToSave = position
                        try positionToSave.save(db)
                        savedPositions.append(positionToSave)
                    }
                }
                
                // Update the in-memory array
                seatingPositions.append(contentsOf: savedPositions)
                print("DEBUG DataStore: Added \(savedPositions.count) seating positions in batch")
                objectWillChange.send()
            } catch {
                print("ERROR DataStore: Error adding seating positions in batch: \(error)")
            }
        }
    ...
    func saveSeatingPosition(_ position: SeatingPosition) -> Bool {
        do {
            // Speichere die Position in der Datenbank
            let savedPosition = try dbManager.saveSeatingPosition(position)
            
            // Aktualisiere den lokalen Cache
            if let index = seatingPositions.firstIndex(where: { $0.id == savedPosition.id }) {
                seatingPositions[index] = savedPosition
            } else {
                seatingPositions.append(savedPosition)
            }
            ...
            print("ERROR DataStore: Fehler beim Speichern der Sitzposition: \(error)")
            return false
        }
    }
}
